...
O modelo neural utilizado foi o \textit{TinyTimeMixer}, versão 2.1, disponível na biblioteca \code{transformers} do \textit{Hugging Face}. Ele é
...
(10\%) e teste (20\%), utilizando a função \textit{prepare\_data\_splits}. A biblioteca \textit{tsfm\_public} é novamente empregada 
para o pré-processamento através da classe \textit{TimeSeriesPreprocessor}, que desta vez é configurada para tratar tanto as variáveis 
alvo (\textit{target\_columns}) quanto as exógenas (\textit{control\_columns}), aplicando a normalização \textit{StandardScaler} em ambas.
