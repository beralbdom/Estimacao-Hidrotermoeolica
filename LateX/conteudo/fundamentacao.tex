...
comparação com o modelo neural. Para ambos os casos, o \code{scikit-learn}, uma biblioteca Python que contém diversos 
algoritmos para análise de dados, incluindo modelos de classificação e regressão, foi utilizada. Esta biblioteca
é construída utilizando as bibliotecas \code{NumPy} e \code{SciPy}, que fornecem suporte para operações
...
O modelo linear utilizado foi o \code{LinearRegression} da biblioteca \code{scikit-learn}. Ele consiste na
...
O modelo de regressão não linear utilizado foi o \code{RandomForestRegressor} da biblioteca \code{scikit-learn}. O modelo 
...
localização e características da usina; e também recuros variáveis no tempo, como índices climáticos. A TSMixer-Ext 
